...
{
	if(filename == NULL || total_num_pts == 0 || centroids == NULL)
	{
		...
		for(j = 0; j < num_procs; j++)
		{
			...
			if(j == 0)
			{
				...
				*num_pts = len;
				...
				// for(i = 0; i < len; i++)
				// {
				// 	printf("RANK - %d, x - %lf, y - %lf\n", myRank, master_points[i][0], master_points[i][1]);
				// }
			...
			else
			{
				...
				for(k = 0; k < len; k++)
				{
					...
					MPI_Send(points[start+k],2,MPI_DOUBLE, j, j, MPI_COMM_WORLD);
				}
			}
			
		}
		printf("Master - Done distributing points\n");
		printf("\n");
	...
	else
	{
		...
		*num_pts = num_elems;
		...
		// Receive points from master, 1 points at a time and store points array
		for (i = 0; i < num_elems; i++)
		{
         	MPI_Recv(points[i], 2 , MPI_DOUBLE, 0, myRank, MPI_COMM_WORLD, &status);
		}
		
		// for(i = 0; i< num_elems; i++)
		// {		
		// 	printf("RANK - %d, x - %lf, y - %lf\n", myRank, points[i][0], points[i][1]);
		// }
		printf("Other proceses - Done receiving points\n");
		printf("\n");
		return points;
